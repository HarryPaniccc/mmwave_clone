...
from scipy.signal import decimate, hilbert,find_peaks
import cv2 as cv
import json
import glob
...
def main():
    ...
    for dog in dogs:
        ...
        for path in glob.glob(os.path.join(working_dir,data_dir_path,dog,"*Exp*")):
            ...
            # heart_rate  = amplitude_threshold_filter(phase_time,phase_data)            
            ...
            peaks_idx,_ = find_peaks(heart_rate)
            ibi = np.diff(phase_time[peaks_idx])
            averaged_rate = moving_average_filter(60/ibi,np.clip(len(ibi)//3,3,25))
            expected_heart_rate = resample_and_sync(polar_time,polar_heart_rate,phase_time[peaks_idx][0:-1])
            
            # plt.plot(phase_time[peaks_idx][0:-1],averaged_rate)
            # plt.plot(phase_time[peaks_idx][0:-1],expected_heart_rate)
            # plt.ylim(40,180)
            # plt.show()
